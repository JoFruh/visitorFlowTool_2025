...
step6_server <- function(id, networkList, SM_pres, SM_noPres, SMcolors, shape, confirm, i18n, currentLang, isFirstRun_stp6, finalPolygons = NULL, versionsUI = list(), triggerStp6 = 0,
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::updateSelectInput(inputId = "languageSelect_6", selected = currentLang)
    ...
    if(r$currentLang == "de"){
      output$bannerUI_6 <- shiny::renderUI({
        imgMap <- imageMap(NS(id, "banner_6"), i18n()$t("www/step5_wsl.png"), list() )
        #replace /" with ', to avoid problems
        return(shiny::tagList(shiny::HTML(gsub( "\"", "'",paste0(imgMap) ))  ) )
      })
    }else if(r$currentLang == "fr"){
      output$bannerUI_6 <- shiny::renderUI({
        imgMap <- imageMap(NS(id, "banner_6"), i18n()$t("www/step5_wsl_fr.png"), list() )
        #replace /" with ', to avoid problems
        return(shiny::tagList(shiny::HTML(gsub( "\"", "'",paste0(imgMap) ))  ) )
      })
    }
    ...
    # })
      ...
      #Language Change ####
      langChangeObs <- observeEvent(input$languageSelect_6, {
        print("CHANGE LANGUAGE")
        if(input$languageSelect_6 == "de"){
        ...
        }else if(input$languageSelect_6 == "fr"){
          ...
          print("FR")
        }else if(input$languageSelect_6 == "en"){
          ...
          print("EN")
        }else if(input$languageSelect_6 == "it"){
...
# print("FINAL CONFIRM")
#         r$confirm <- input$confirmButton6
#         return(list(pathUsage = shiny::reactive({r$pathUsage}), networkList = shiny::reactive({r$networkList}), confirm = shiny::reactive({r$confirm}), newVersions = shiny::reactive({input$newVersionsButton}), trigger = shiny::reactive(r$triggerStp6), versionsUI = shiny::reactive(versionsUI) ) )
      ...
      obsNewVersions <- shiny::observeEvent(input$newVersionsButton, {
        ...
        shiny::removeUI(selector = "div#placeholder_step6")
        shiny::insertUI(selector = "#topPlaceHolder",
                        ui = shiny::tags$div(
                          id = "placeholder_step6"
                        )
        )
